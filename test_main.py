...
LOF_directory = './output/'    
LOF_file = LOF_directory + 'LOF_output.csv'                                                                  # Output csv
LOF_detect(LOF_input, LOF_file)
LOF = pd.read_csv('./output/LOF_output.csv')
# 將相同 file_name 的 time_rel(sec) 存成 array
result = LOF.groupby('filename')['time_rel(sec)'].apply(list).reset_index()
result_abs = LOF.groupby('filename')['time_abs(%Y-%m-%dT%H:%M:%S.%f)'].apply(list).reset_index()
# 如果你想把結果轉換成字典，可以這樣做
